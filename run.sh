...
mkdir -p "$OUT"
rm -f "$OUT"/run_sym*.csv    # stare behy nech sa nepomiesaju do glob-u

for sym in 0 1 2; do
    if [ "$NCOLS" -eq 11 ]; then
        for w in flow packet byte; do
            echo "sym=$sym weighting=$w"
            ./benchmark "$DATA" "$OUT/run_sym${sym}_${w}.csv" "$sym" 0 "$w"
        done
    else
        echo "sym=$sym (simple tuple)"
        ./benchmark "$DATA" "$OUT/run_sym${sym}_flow.csv" "$sym" 0
    fi
done
